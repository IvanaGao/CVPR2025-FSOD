...
class OVDINO(nn.Module):
    ...
    Args:
        backbone (nn.Module): visual_backbone module
        language_backbone (nn.Module): language visual_backbone module
    ...
    def __init__(
        ...
        # inference_template: str = "full",
        app_mode: bool = False,
        use_pn_dist_loss: bool = False,
    ):
        super().__init__()
        # define visual_backbone and position embedding module
        ...
        text_embed_dim = self.language_backbone.lang_model.config.hidden_size   # 兼容不同尺寸bert
        ...
        # denoising 参数
        # 预训练模型中num_classes默认为150，但是对于coco数据集只有80，会导致模型权重加载或者forward异常
        # 1、如果不修改该参数（也就是配置文件中设置150），模型能正常加载，但是在forward中由于coco只有80类，
        # 导致text_embed.view(batch_size, num_classes, -1)会发生异常
        # 2、如果在配置文件中直接修改为80，那么创建模型后加载预训练权重会出现异常，如果忽略该部分权重又会影响效果
        # 解决方案：
        # 对于coco数据集微调，创建模型时 num_classes = 150，模型创建后再将model.num_classes修改为相应参数（其他数据集不需要处理）
        self.label_enc = nn.Embedding(num_classes, embed_dim)   # 预训练完毕后该参数是固化的，不建议重新训练
        ...
        # two-stage（为什么将属于decoder的模型放在这里构建）
        ...
        #  store the same text_embed during evaluation to speed up.（这会影响动态caption输入）
        ...
        self.use_pn_dist_loss = use_pn_dist_loss

    def reset_text_embed_dict(self):
        self.text_embed_dict = defaultdict()

    def forward(self, batched_inputs, use_copy_paste, cp_threshold):
        ...
        if use_copy_paste:
            ...
            if self.training:
                # copy-paste augmentation
                images, names = self.preprocess_image_and_name_train(batched_inputs, cp_threshold)  # 像素归一化 + tensor化 + 类别是否添加template
            else:
                images, names = self.preprocess_image_and_name(batched_inputs)  # 像素归一化 + tensor化 + 类别是否添加template
        ...
        if self.training:
            ...
            assert (batch_size == 1), f"batch_size={batch_size}, only support single image inference now..."
        ...
        if not self.training:
            ...
            if self.text_embed_dict.get(chunk_index, None) is None or self.app_mode:
                if isinstance(names[0], list):
                    # 目的是兼容一个类别多个prompt
                    text_embed = torch.stack([self.language_backbone(name) for name in names], dim=0)  # [bs*num_classes, num_templates, embed_dim]
            ...
            #     text_embed_list = []
            #     for name in names:
            #         if self.text_embed_dict.get(name[0], None) is None:
            #             name_embed = self.language_backbone(name)
            #             text_embed_list.append(name_embed)
            #             self.text_embed_dict[name[0]] = name_embed
            #         else:
            #             text_embed_list.append(self.text_embed_dict[name[0]])
            #     text_embed = torch.stack(text_embed_list, dim=0)  # [bs*num_classes, num_templates, embed_dim]
            #     text_embed = text_embed.mean(1)
            # else:
            #     import sys
            #     sys.exit(1)
        ...
        num_classes = len(batched_inputs[0]['category_names'])
        text_embed = text_embed.view(batch_size, num_classes, -1)

        # project visual_backbone features to the reuired dimension of transformer, we use multi-scale features in DINO
        ...
        # denoising preprocessing, prepare label query embedding
        if self.training:
        ...
        # feed into transformer  enc_reference,  # [0..1]
        ...
        interm_class = self.transformer.decoder.class_embed[self.transformer.decoder.num_layers](enc_state, text_embed)
        ...
        if self.training:
            ...
            # 添加自定义损失
            if self.use_pn_dist_loss:
                pn_cos_dist = 0
                # 1、正负类距离损失
                for i, tgt in enumerate(targets):
                    indexes = tgt['labels'].to(text_embed.device)
                    if len(indexes) == 0:
                        continue
                    positive_vector = torch.mean(text_embed[i][indexes], dim=0)
                    mask = torch.ones(text_embed.size(1), dtype=bool, device=text_embed.device)
                    mask[indexes] = False
                    negative_vector = torch.mean(text_embed[i][mask], dim=0)
                    pn_cos_dist += torch.cosine_similarity(positive_vector.unsqueeze(0), negative_vector.unsqueeze(0), dim=-1)[0]
                pn_cos_dist = pn_cos_dist / len(targets) if len(targets) > 0 else 0
                loss_dict['loss_pn_dist'] = pn_cos_dist

            return loss_dict
        else:
            box_cls = output["pred_logits"]
            box_pred = output["pred_boxes"]
            results = self.inference(box_cls, box_pred, images.image_sizes, category_start_index)
            processed_results = []
    ...
    def preprocess_image_and_name(self, batched_inputs):
        ...
        category_names = [x["category_names"] for x in batched_inputs]  # list[list[str]]
        if not self.training and self.inference_template != "identity":
            category_names = [
                [
                    [
                        # template.format(clean_words_or_phrase(cat_name))  # 有bug
                        template.format(clean_words_or_phrase(cat_name[0]))
    ...
    def preprocess_image_and_name_train(self, batched_inputs, cp_threshold):
        COPY_PASTE_PROB = cp_threshold
        IGNORE_THR = 0.3

        images_fill = []
        for sample in batched_inputs:

            gt_boxes = sample['instances'].gt_boxes.tensor.tolist()
            gt_classes = sample['instances'].gt_classes.tolist()

            image = sample['image']
            bboxes = []
            cats = sample['instances'].gt_classes.tolist()
            for bbox in sample['instances'].gt_boxes:
                x1, y1, x2, y2 = bbox
                x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
                bboxes.append([x1, y1, x2, y2]) # x1x2y1y2
            ...
            for bbox, cat in zip(bboxes, cats):
                if random.random() < COPY_PASTE_PROB:
                    x1, y1, x2, y2 = bbox
                    # scale the bbox area by 10% on each side to copy
                    # x1, y1, x2, y2 = int(x1 * 0.9), int(y1 * 0.9), int(x2 * 1.1), int(y2 * 1.1)
                    x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
                    w, h = x2 - x1, y2 - y1

                    # for _ in range(5):
                    #     print(max_y - h - 1, max_x - w - 1)
                    #     print("---------------------")

                    if w / max_x > IGNORE_THR or h / max_y > IGNORE_THR:
                        continue

                    tl_y = random.randint(0, max_y - h - 1)
                    tl_x = random.randint(0, max_x - w - 1)

                    to_copy = image[:, y1:y2 + 1, x1:x2 + 1]
                    to_copy_h = to_copy.shape[1]
                    to_copy_w = to_copy.shape[2]

                    # image_new[:, tl_y:tl_y + h + 1, tl_x:tl_x + w + 1] = image[:, y1:y2 + 1, x1:x2 + 1]
                    image_new[:, tl_y:tl_y + to_copy_h, tl_x:tl_x + to_copy_w] = image[:, y1:y2 + 1, x1:x2 + 1]
                    gt_boxes.append([tl_x, tl_y, tl_x + w, tl_y + h])
                    gt_classes.append(cat)

            gt_boxes = Boxes(torch.tensor(gt_boxes))
            gt_classes = torch.tensor(gt_classes)

            instances = Instances(image_size=(max_y, max_x))
            ...
            #
            # # Step 2: Transpose to HxWx3
            # image_new = image_new.transpose(1, 2, 0)
            #
            ...
            #     color = random.choice(color_names)  # 随机选择一个颜色
            ...
            images_fill.append(image_new)

        images = [self.normalizer(image.to(self.device)) for image in images_fill]
        images = ImageList.from_tensors(images)
        # process category_names
        category_names = [x["category_names"] for x in batched_inputs]  # list[list[str]]
        if not self.training and self.inference_template != "identity":
