        ...
        **kwargs allow passing a more general data flow when combining
        ...
        Args:
            query (torch.Tensor): Query embeddings with shape
                `(num_query, bs, embed_dim)` if self.batch_first is False, else `(bs, num_query, embed_dim)`
            key (torch.Tensor): Key embeddings with shape
                `(num_key, bs, embed_dim)` if self.batch_first is False, else `(bs, num_key, embed_dim)`
            value (torch.Tensor): Value embeddings with the same shape as `key`.
                Same in `torch.nn.MultiheadAttention.forward`.
                Default: None. If None, the `key` will be used.
            ...
            key_pos (torch.Tensor): The position embedding for key.
                Default: None.
                If None, and `query_pos` has the same shape as `key`, then `query_pos` will be used for `key_pos`.
        ...
        if key_pos is not None:
            key = key + key_pos
        gate_query = self.attn(query, key, value, attn_mask=attn_mask, key_padding_mask=key_padding_mask)[0]
        # 门控残差连接
        ...
        **kwargs contains the specific arguments of attentions.
        ...
        Args:
            query (torch.Tensor): Query embeddings with shape
                ...
                which should be specified follows the attention module used in `BaseTransformerLayer`.
            ...
            attn_masks (List[Tensor] | None): A list of 2D ByteTensor used
                ...
                `attn_masks` should be equal to the number of `attention` in `operation_order`.
                Default: None.
    ...
    def forward(
        ...
        **kwargs,
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            query_sine_embed = get_sine_pos_embed(reference_points_input[:, :, 0, :],  num_pos_feats=query.size(-1) // 2)
...
class DINOTransformer(nn.Module):
    ...
    def get_reference_points(spatial_shapes, valid_ratios, device):
        ...
        """
        # valid_ratios解析参考 https://github.com/open-mmlab/mmdetection/issues/8656#issuecomment-1295866151
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 获取所有尺度像素的坐标点，该参考点用于deformable attention，而不是目标空间提议
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=feat.device)
        # image 多次度feature maps deformable attention 编码
        memory = self.encoder(
            query=feat_flatten,
            key=None,
            value=None,
        ...
        # output_proposals: bs, num_tokens, 4. unsigmoided. # 每一个image token都有一个基准bbox, xywh，其中wh为1

        # normal.
        enc_outputs_class = self.decoder.class_embed[self.decoder.num_layers](output_memory, class_embed)
        # enc_outputs_class = torch.einsum("bnd,bcd->bnc", output_memory, class_embed)
        enc_outputs_coord_unact = (self.decoder.bbox_embed[self.decoder.num_layers](output_memory) + output_proposals)  # unsigmoided.
        ...
        topk_coords_unact = torch.gather(enc_outputs_coord_unact, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 4))  # unsigmoided.
        reference_points = topk_coords_unact.detach().sigmoid()
        if query_embed[1] is not None:
            # query_embed 是dino部分的object query，需要与select object query一起forward，数量为170
