...
class YOLOXHSVRandomAug(Augmentation):
    """
    Outputs an image by cropping a random "subrect" of the source image.
    ...
    def __init__(self, hue_delta=5, saturation_delta=30, value_delta=30):
        """
        Args:
            ohue_delta (int): delta of hue. Defaults to 5.
    ...
    def get_transform(self, image):
        ...
        h_gain, s_gain, v_gain = hsv_gains

        return YOLOXHSVRandomAugTransform(
            h_gain=h_gain,
            s_gain=s_gain,
            v_gain=v_gain
        )
...
class YOLOXHSVRandomAugTransform(Transform):
    """
    Transforms pixel colors with PIL enhance functions.
    """

    def __init__(self, h_gain, s_gain, v_gain):
        super().__init__()
    ...
    def apply_image(self, img: np.ndarray) -> np.ndarray:
        ...
        for i in range(img.shape[0]):
            for j in range(img.shape[1]):
                ...
                h = (h + self.h_gain) % 1.0  # h in [0, 1)
                s = np.clip(s + self.s_gain, 0, 1)
                v = np.clip(v + self.v_gain, 0, 1)
                hsv_image[i, j] = colorsys.hsv_to_rgb(h, s, v)

        # image_bgr = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        #
        # # Convert to HSV
        # img_hsv = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2HSV).astype(np.int16)
        #
        # # Apply HSV gains
        # img_hsv[..., 0] = (img_hsv[..., 0] + self.h_gain) % 180
        # img_hsv[..., 1] = np.clip(img_hsv[..., 1] + self.s_gain, 0, 255)
        # img_hsv[..., 2] = np.clip(img_hsv[..., 2] + self.v_gain, 0, 255)
        #
        # # Convert back to uint8
        # img_hsv = hsv_image.astype(np.uint8)

        # # Convert back to BGR and then RGB
        # # image_bgr = cv2.cvtColor(img_hsv, cv2.COLOR_HSV2BGR)
